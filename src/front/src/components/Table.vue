      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="delConfirm" persistent max-width="800">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="green darken-1" text @click="doneDel()">Cancel</v-btn>
              <v-btn color="green darken-1" text @click="deleteItem(currentDelItem, true)">Approve</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-toolbar>
...
<script>
  export default {
    ...
    methods: {
      ...
      deleteItem (item, forced) {
        this.currentDelItem = item;
        forced ? this.$emit('del', item, true) : (confirm('Are you sure you want to delete this item?') && this.$emit('del', item));
      },
